...
public class ModLootTableModifiers {
    ...
    public static final LootCondition.Builder WITH_HOE = MatchToolLootCondition.builder(ItemPredicate.Builder.create().tag(ItemTags.HOES));
    ...
    public static void modifyLootTables() {
        ...
        //MAKE OAK LEAVES DROP MORE APPLES WITH A HOE
        LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
            if(Blocks.OAK_LEAVES.getLootTableId().equals(id)) {
                LootPool.Builder poolBuilder = LootPool.builder()
                        .rolls(ConstantLootNumberProvider.create(1))
                        .conditionally(RandomChanceLootCondition.builder(0.05f).and(WITH_HOE))
                        //Drops from roughly one in 20 blocks (~thrice per tree) provided a hoe is used
        ...
        //MAKE DARK OAK LEAVES DROP MORE APPLES WITH A HOE
        LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
            if(Blocks.DARK_OAK_LEAVES.getLootTableId().equals(id)) {
                LootPool.Builder poolBuilder = LootPool.builder()
                        .rolls(ConstantLootNumberProvider.create(1))
                        .conditionally(RandomChanceLootCondition.builder(0.05f).and(WITH_HOE))
                        //Drops from roughly one in 20 blocks provided a hoe is used
                        .with(ItemEntry.builder(Items.APPLE))
                        .apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1.0f, 1.0f)).build());
